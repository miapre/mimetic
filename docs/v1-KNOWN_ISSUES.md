...
| **Team/org library with components + typography variables (no text styles)** | Full | Since v1.8.0, DSs that publish typography as variables (`Font size/*`, `Line height/*`, `Font weight/*`) work in strict mode without text styles. |
...
Donut charts with `categoryVariables` (DS color variable paths per segment) may fail in strict mode due to a binding issue. Workaround: use native `create_ellipse` with `arcData` and `fillVariable` per segment for full DS compliance on donuts.
