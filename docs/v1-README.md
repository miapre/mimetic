...
Every build enforces 60 quality rules. You don't configure them. They just run.
...
- **Strict DS enforcement by default.** Since v1.6.0, all builds run in strict mode — raw hex, raw px, and raw fonts are rejected. Design systems with typography variables (instead of text styles) are supported since v1.8.0.
- **Claude-optimized.** The 60-rule governance and learning reports work best with Claude Code. Other MCP clients get the tools but may not follow the full protocol.
