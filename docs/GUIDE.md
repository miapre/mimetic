...
## Part 3 — Set up the Write Bridge (figma-write-mcp)
...
Inside your main project folder, create a subfolder called `figma-write-mcp`. In a terminal:

```bash
mkdir figma-write-mcp
cd figma-write-mcp
```

### 3.2 Create package.json

Create a file called `package.json` inside `figma-write-mcp`:

```json
{
  "name": "figma-write-mcp",
  "version": "1.0.0",
  "type": "module",
  "scripts": {
...
Create a file called `.env` (the dot at the start is intentional) in the `figma-write-mcp` folder:
...
### 3.7 Register figma-write-mcp with Claude Code
...
```json
{
  "mcpServers": {
    ...
    "figma-write-mcp": {
      "command": "node",
      "args": ["/absolute/path/to/your/figma-write-mcp/mcp.js"]
    }
  }
}
```
...
3. Navigate to your `figma-write-mcp/plugin/` folder and select `manifest.json`
4. The plugin now appears under **Plugins → Development → Figma Write Bridge**
...
1. In Figma desktop, go to **Plugins → Development → Figma Write Bridge → Run**
...
→ The bridge is running but the Figma plugin is not. Go to Figma desktop → Plugins → Development → Figma Write Bridge → Run. The bridge terminal should print "plugin connected".
...
- [ ] `npm install` run inside `figma-write-mcp/`
...
- [ ] Bridge running: `npm run bridge` in `figma-write-mcp/`
