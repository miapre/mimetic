...
1. **Phase 0 — Target** (Platform Architect): Confirm target file/node, calculate artboard placement, identify variable mode requirements. **DS-only enforcement (Rule 43):** Call `set_session_defaults` with `dsMode: "strict"` at the start of every build. The plugin now defaults to strict mode and will reject `dsMode: "permissive"` if the DS has published tokens (validated at the API level). Once set to strict, the mode is immutable for the session — it cannot be downgraded mid-build. Permissive mode is ONLY accepted for component-only DSs with zero published variables and styles.
   ...
   **Color mode (mandatory):** `mimic_pipeline_resolve` returns a `colorScheme` field ("dark" or "light") based on the HTML's background color. After creating the artboard, call `figma_set_variable_mode` to match the HTML's color scheme. If the pipeline returns `colorScheme: "dark"`, set `modeIndex: 1` (Dark). If "light" or null, set `modeIndex: 0` (Light). Never skip this step — building a dark HTML with light-mode DS tokens produces an inverted artboard.

   **Batch size limit:** Keep `figma_batch` operations to 6 or fewer per call. Larger batches risk bridge disconnects, and retrying a failed batch duplicates any operations that succeeded before the disconnect.
...
2. **Phase 1 — DS Discovery** (DS Integration Engineer): **Mandatory first step: call `mimic_ai_knowledge_read` to load the DS knowledge base (Rule 48).** Then produce a component map: `HTML element → DS component key` or `"primitive" + reason`. For VERIFIED patterns in the knowledge base, validate the component key still resolves before using it. This is NON-OPTIONAL — skipping it is a critical violation (Rule 23). The plugin now BLOCKS artboard creation if no discovery has been performed (returns `DS_DISCOVERY_REQUIRED` error, not warning). **Regression check (Rule 30):** compare against previous builds of similar screen types.
   ...
   **Composite component matching (Rule 58):** After individual component mapping, analyze the HTML for composite sections (header, footer, sidebar). For each:
   1. **Extract features** from the HTML section: count nav links, detect search input, avatar, logo, CTA buttons, dropdowns, etc.
   2. **Search the DS** for matching composite components (e.g., "Header", "Navigation", "Footer"). Use the normalized DS knowledge to find component sets.
   3. **Inspect all variants** of the matching component (`get_component_variants` + `get_node_props`). List all boolean toggles and variant axes — these represent the component's feature set.
   4. **Compare features**: does any variant combination cover all the HTML features? Layout differences (logo left vs right, links centered vs left) are NOT disqualifying — only missing features (e.g., HTML has search but component has no search toggle).
   5. **Decide**: if a variant covers all features → use the component. If not → build with primitives.
   6. **Record the analysis** for the build report: `componentFitAnalysis` with `htmlFeatures`, `componentFeatures`, `matchResult`, `missingFeatures`, and `recommendations`.

   Recommendations must be specific and constructive: "Would adding a 'Search' boolean property to your Header Navigation component be useful? It would enable Mimic to use the component for headers with search inputs."

   Successful composite matches are cached in ds-knowledge as `composite/header`, `composite/footer`, etc. Future builds skip the full variant inspection for known matches.
...
**Authoritative source:** `GOLDEN_RULES.md` (60 rules).
...
### Charts — Build Protocol (Rule 25)

Charts can be built two ways: (1) `figma_create_chart` for rapid builds — allowed in strict mode since v1.7.0, uses session defaults for text and DS variable params for colors; (2) native primitives for maximum DS compliance. Both are acceptable. **Never switch to `use_figma` to build charts** — `create_chart` or native primitives via Mimic tools are the only options. Native build tools:
...
- `create_chart` — chart tool, allowed in strict mode (uses session defaults + DS variable params for colors/labels)
...
## Rule 55: No tool switching during builds

Once a Mimic build session starts (artboard created via `create_frame`), the ONLY Figma write tools allowed are Mimic's own tools (`figma_create_frame`, `figma_create_text`, `figma_insert_component`, `figma_batch`, `figma_create_chart`, `figma_create_svg`, `figma_create_ellipse`, `figma_create_rectangle`, `figma_set_*`, `figma_move_node`).

**`mcp__claude_ai_Figma__use_figma` is FORBIDDEN during an active Mimic build session.**

If a Mimic tool returns a `DS_STRICT_VIOLATION` error:
1. Check if `insert_component` can solve it (components have their own text/styles)
2. Check if a different Mimic tool can solve it (e.g., `create_frame` instead of manual construction)
3. If no Mimic tool can solve it: **STOP.** Report the blocker to the user
4. **NEVER switch to `use_figma` as a workaround** — it bypasses all DS enforcement and destroys compliance for the entire build

This rule exists because `use_figma` has zero DS enforcement. A build with one unstyled text node via Mimic is infinitely better than a build with zero DS compliance via `use_figma`.

## Rule 56: Typography-variables-only DSs

Some design systems publish typography as variables (`Font size/*`, `Line height/*`, `Font weight/*`) instead of Figma text styles. When the variable cache contains typography variables, strict mode **requires** `fontSizeVariable` and `lineHeightVariable` params on `create_text` — raw `fontSize`/`lineHeight` numbers are rejected. The plugin binds these via `setBoundVariable()`. Pass variable paths like `"Font size/display-2xl"`, `"Font weight/semibold"`, `"Line height/text-md"`. The `fillVariable` requirement still applies — text color must always use a DS variable.

## Rule 57: Mandatory role checkpoint before toolchain switch

Before switching away from Mimic tools for ANY reason during a build, a mandatory role review must occur. The DS Integration role and Product QA role have VETO power over tool switching. If either vetoes, the build stops and the blocker is reported to the user. Unilateral tool switching is a critical violation.
