...
### Added
- **Typography-variables-only DS support (Rule 55/60)** -- Design systems that publish typography as variables (`Font size/*`, `Line height/*`, `Font weight/*`) instead of Figma text styles now work in strict mode. When the variable cache contains typography variables but no text styles exist, `create_text` accepts `fontSize`/`fontWeight`/`lineHeight` params as DS-compliant. Reports `textStyle: "typography_variables"` in dsCompliance.
- **Charts allowed in strict mode** -- `figma_create_chart` no longer throws `CHART_BLOCKED_STRICT_MODE`. Charts use session defaults for text and DS variable params for colors. Internal chart text is tagged `[CHART]` and skipped by `validate_ds_compliance`.
- **Anti-bypass enforcement on all errors (Rule 56)** -- Every `DS_STRICT_VIOLATION`, `DS_VARIABLE_NOT_FOUND`, and `DS_TEXT_STYLE_FAILED` error now includes an `ANTI_BYPASS_SUFFIX` warning the orchestrator not to switch to `use_figma` or other non-Mimic tools.
- **Component search gate upgraded to blocking error (Rule 59)** -- `COMPONENT_SEARCH_MISSING` is now `COMPONENT_SEARCH_REQUIRED`: returns an error and does NOT create the frame. Forces `insert_component` to be called before any child frame creation when VERIFIED patterns exist.
- **Rule 55: No tool switching during builds** -- `mcp__claude_ai_Figma__use_figma` is explicitly forbidden during active Mimic build sessions. If a Mimic tool is blocked, the build must STOP — not switch tools.
- **Rule 57: Mandatory role checkpoint before toolchain switch** -- DS Integration and Product QA roles have veto power over any tool switch decision.
- **Rules 59–60 in GOLDEN_RULES.md** -- Anti-bypass and typography-variables-only rules with full incident context from the 2026-04-30 enforcement breach.
- **51 → 60 golden rules.**

### Fixed
- **Text creation deadlock** -- Strict mode previously required `textStyleId` even when the DS had no text styles (only typography variables). This created a deadlock that forced the LLM to bypass the entire Mimic toolchain. Now resolved by detecting typography variables in the cache.
- **Chart creation deadlock** -- Strict mode previously blocked `figma_create_chart` entirely, pushing the LLM to build charts via `use_figma` with zero DS compliance. Now allowed with DS variable params.
- **CLAUDE.md chart contradiction** -- "Charts — Native Build Protocol" section previously said "never use `figma_create_chart` for production builds" which contradicted the chart unblock. Updated to allow both `create_chart` and native primitives.
- **ANTI_BYPASS_SUFFIX consistency** -- One error (textStyleId missing, line 949) used inline anti-bypass text instead of the constant. Now uses the shared constant for maintainability.

## 1.7.0 (2026-04-30)

### Added
- **MCP-level build session tracking** -- New `buildSession` state object in mcp.js tracks which mandatory phases have been completed: knowledge read, component search, artboard creation, build report generation. Reset on `set_session_defaults` (new build session).
- **`PHASE1_KNOWLEDGE_REQUIRED` gate (Rule 48 enforcement)** -- When the knowledge base has active patterns, creating an artboard without calling `mimic_ai_knowledge_read` first returns a blocking error. Prevents builds from ignoring known component mappings. Cold starts (empty knowledge base) pass through.
- **`COMPONENT_SEARCH_MISSING` warning (Rule 50)** -- When content frames are created inside an artboard but `insert_component` was never called, the first child frame returns a non-blocking warning listing all VERIFIED component mappings being ignored. Fires once per session.
- **`PHASE5_SKIPPED` warning (Rule 51)** -- When `set_session_defaults` starts a new build session while the previous session created an artboard but never called `mimic_generate_build_report`, a warning is injected into the response. Also surfaced in `mimic_status` as `build_session.phase5_pending`.
- **Build session state in `mimic_status`** -- New `build_session` field shows `knowledge_read`, `component_search_done`, `artboard_created`, `build_report_generated`, and `phase5_pending` flags. Status message warns when Phase 5 is pending.
- **Rule 50: Component search is mandatory** -- Primitives are the fallback, not the default. Orchestrator must check knowledge base + search DS for all component types before building with primitives.
- **Rule 51: Phase 5 is structurally enforced** -- A build without a report is incomplete. Enforcement via `PHASE5_SKIPPED` warning and `mimic_status` surfacing.
- **49 → 51 golden rules** -- Updated GOLDEN_RULES.md, CLAUDE.md, ROLES.md.
